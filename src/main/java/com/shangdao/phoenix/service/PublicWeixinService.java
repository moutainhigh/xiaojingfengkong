...
public class PublicWeixinService {
	...
	public Object getApi(HttpServletRequest req){
		if (!publicEnable) {
			throw new OutsideRuntimeException(ExceptionResultEnum.PBWX_START_ERROR);
		}
		try {
...
//				String sEchoStr = wxcpt.verifyUrl(sVerifyMsgSig, sVerifyTimeStamp, sVerifyNonce, sVerifyEchoStr);
	...
	private Map _getOauth2Template(String url) {
		if (!publicEnable) {
			throw new OutsideRuntimeException(ExceptionResultEnum.HAVE_NO_START_PUBLIC_WEIXIN_FUNCTION);
		}
		RestTemplate restTemplate = new RestTemplate();
	...
	protected Map _postTemplate(String url, Map params) {
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			throw new OutsideRuntimeException(ExceptionResultEnum.POST_TO_WEIXIN_ERROR);
		}
	...
	private Map stringToMap(String str) {
		try {
			...
			// TODO Auto-generated catch block
			throw new OutsideRuntimeException(ExceptionResultEnum.USER_WEIXIN_RETURN_JSON_ERROR);
		}
	}
