...
public class PostMethodService extends AbstractPostGetMethodService {
	...
	public HTTPResponse postDispatch(String entityName, String act, String mode, String body) {
		...
		if (terminalStr == null) {
			throw new OutsideRuntimeException(ExceptionResultEnum.HAVE_NO_TERMINAL);
		}

		try {
			Terminal.valueOf(terminalStr);
		} catch (IllegalArgumentException e) {
			throw new OutsideRuntimeException(ExceptionResultEnum.TERMINAL_ERROR);
		}
...
//		}
	...
	private long _template(PostMethodWrapper postMethodWrapper, String act, Object postBody, Object oldInstance) {
		...
		//保存之后执行service自定义钩子
		_afterHook(postMethodWrapper, act, newInstance);
		// 写入log记录
		...
		if (files != null && !files.isEmpty()) {
			...
			try {
				for (OssImage ossImage : files) {
				    if(ossImage.getImgId() != null && ossImage.getImgId() != 0L){
				        continue;
				    }
	...
	private void _fillJobDataMap(ActNotice actNotice, JobDataMap jobDataMap) {
		...
		if (delayType.equals(DelayType.DELAY)) {
			if (actNotice.getCanBeCancelled()) {
	...
	private ILog _log(PostMethodWrapper postMethodWrapper, String act, Object newInstance, BeforeSave beforeSave) {
		if (postMethodWrapper.getStructure().isLogEntity()) {
			...
			try {
				...
				e.printStackTrace();
				throw new OutsideRuntimeException(ExceptionResultEnum.LOG_SAVE_ERROR);
			}
	...
	private void  _afterHook(PostMethodWrapper postMethodWrapper, String act, Object newInstance){
	    if (postMethodWrapper.getStructure().getEntityService() != null) {
            try {
                Method declaredMethod = postMethodWrapper.getStructure().getEntityService().getClass()
                        .getDeclaredMethod(MethodNameFormatUtil.formatName("after"+act), PostMethodWrapper.class, Object.class);
                declaredMethod.invoke(postMethodWrapper.getStructure().getEntityService(), postMethodWrapper, 
                        newInstance);
            } catch (NoSuchMethodException | SecurityException e) {
	...
	private Object _save(PostMethodWrapper postMethodWrapper, String act, Object postObject, Object oldInstance) {
		...
		try {
				...
				for (Entry<String, ColumnStucture> entry : entrySet) {
					try {
						if (entry.getValue().getJoinType().equals(JoinType.ONE_TO_ONE)
							...
							if (propertyValue !=null && propertyValue.equals(0L)) {
			...
			e.printStackTrace();
			throw new OutsideRuntimeException(ExceptionResultEnum.DATA_SAVE_ERROR);
		}
		return newInstance;
	...
	public static class PostMethodWrapper implements Serializable {
		...
		public PostMethodWrapper(String entityName, String act, String mode, String body) {
			...
			try {
				...
				if (act.equals("create")) {
					if (!id.equals(0L)) {
						throw new OutsideRuntimeException(ExceptionResultEnum.CREATE_NOT_NEED_ID);
					}
				} else if (id.equals(0L)) {
					throw new OutsideRuntimeException(ExceptionResultEnum.HAVE_NO_ID);
				}

			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				throw new OutsideRuntimeException(ExceptionResultEnum.POST_DATA_ERROR);
			}
