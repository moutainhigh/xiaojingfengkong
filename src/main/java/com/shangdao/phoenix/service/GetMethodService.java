...
import com.shangdao.phoenix.util.*;
...
//		}
		...
		if (act.equals("list")) {
            ...
            response = _hook(getMethodWrapper, "table", table(getMethodWrapper));
        } else {
        ...
        if (act.equals("table")) {
            ((HttpTableResponse) response).buildTable();
        }
	...
	public HTTPResponse myInfo() {
		...
		params.put("fields", "*,state,departments,roles.menus,roles.buttons");
		...
		List<Map> menus = (List<Map>) menuResponse.getData();
		List buttonIds = (List) roles.stream().flatMap(GetMethodService::buttonList).distinct()
				.map(e -> ((Map) e).get("id").toString()).collect(Collectors.toList());
		roles.stream().forEach(e -> ((Map) e).remove("buttons"));
		for (Map menu : menus) {
			Long menuId = (Long) menu.get("id");
			menu.put("buttons", getButtons(menuId, buttonIds));
			List children = (List) menu.get("children");
			if (children != null) {
				for (Object child : children) {
					Long childMenuId = (Long) ((Map) child).get("id");
					((Map) child).put("buttons", getButtons(childMenuId, buttonIds));
				}
			}
		}
	...
	private Object getButtons(long menuId, List buttonIds) {

		HashMap<String, String> buttonParams = new HashMap<>();
		buttonParams.put("menu.id", String.valueOf(menuId));
		buttonParams.put("id@", String.join(",", buttonIds));
		HTTPResponse buttonResponse = getDispatch("button", "list", "all", buttonParams,
				new GetMethodService.MaxItemOnePage());
		return buttonResponse.getData();
	}
	...
	private static Stream buttonList(Map map) {
		Object buttons = map.get("buttons");
		if (buttons instanceof List) {
			return ((List) buttons).stream();
		}
		return Collections.EMPTY_LIST.stream();
	}
    ...
    private HttpTableResponse table(GetMethodWrapper getMethodWrapper) {
        HTTPListResponse response = list(getMethodWrapper);
        List<Map<String, Object>> data = (List<Map<String, Object>>) response.getData();
        AbstractTable table = new SimpleTable();
        table.setHead(getMethodWrapper.getParams().get("fields"));
        table.setBody(data);
        return new HttpTableResponse(table, response.getTotal(), response.getPageSize(), response.getPageNumber());
    }
		...
		if (countStartAndSizeOfList == null) {
			...
			if (getMethodWrapper.getStructure().isLogEntity()) {
				...
				if (getMethodWrapper.getStructure().isStateMachineEntity()) {
					List Logs = (List) detail.get("logs");
	...
	private HTTPListResponse group(GetMethodWrapper getMethodWrapper) {
		...
		if (preparedParams.get("group") == null) {
			throw new OutsideRuntimeException(ExceptionResultEnum.GROUP_HAVE_NO_PARAM);
		}
	...
	private static Integer comparingByFileAct(Map<String, Object> map) {
		if (((Map) map.get("act")).get("sortNumber") == null) {
			return 0;
		}
		...
		public GetMethodWrapper(String entityName, String act, String mode, Map<String, String> params, Pageable page) {
			...
			if (params != null && params.get("id") != null) {
				try {
					...
					e.printStackTrace();
					throw new OutsideRuntimeException(ExceptionResultEnum.ID_ERROR);
				}
			}
			this.params = params;
			this.page = page;
