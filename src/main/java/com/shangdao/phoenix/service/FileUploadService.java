...
public class FileUploadService {
	...
	public HTTPResponse upload(MultipartFile file) {
		...
		try {
			...
			e.printStackTrace();
			throw new OutsideRuntimeException(ExceptionResultEnum.FILE_UPLOAD_ALIYUN_ERROR);
		}
	...
	private OssImage fileInformation(MultipartFile file) {
		...
		try {
			...
			e.printStackTrace();
			throw new OutsideRuntimeException(ExceptionResultEnum.FILE_FORMAT_PARSE_ERROR);
		}
	...
	private void transferImage(OssImage fileInformation, InputStream fileInputStream) {
		...
		try {
			...
			throw new OutsideRuntimeException(ExceptionResultEnum.ALIYUN_FILE_FORMAT_ERROR);
		} finally {
			ossClient.shutdown();
		}
	}
	...
	private ImageInfo[] picutureResize_198_720(OSSClient ossClient, String transFileNameTemplate,
		...
		try {
			...
			throw new OutsideRuntimeException(ExceptionResultEnum.ALIYUN_IMG_FORMAT_ERROR);
		}
		// return imageInfos;
	}
