...
import com.shangdao.phoenix.entity.report.module.OverdueCreditModule;
 ...
 * @date 2018/04/03
 */
public class OverdueCreditModuleStrategy extends SingleAPIModuleStrategy {
    private static final Logger logger = LoggerFactory.getLogger(OverdueCreditModuleStrategy.class);

    private final static long API_ID = 17L;

    public OverdueCreditModuleStrategy(Report report, SupplyAPIRepository supplyAPIRepository, APISearchRepository apiSearchRepository) {
    ...
    public BaseModule analyseData() {
        OverdueCreditModule overdueCreditModule = new OverdueCreditModule(Color.SUCCESS);
        if (apiResponse != null) {
            if ("0000".equals(apiResponse.get("rc").toString())) {
                ...
                for (Object o : list) {
                    ...
                    if ("B".equals(blackRiskItem.get("blackRiskType"))) {
                ...
                if (blackRisks.size() > 0) {
                    overdueCreditModule.setColor(Color.DANGER);
                }
                overdueCreditModule.setBlackRisks(blackRisks);
            } else {
                overdueCreditModule.setColor(Color.ERROR);
            }
        } else {
            overdueCreditModule.setColor(Color.TIMEOUT);
        }
    ...
    public void setModuleIntoReport(BaseModule module) {
        report.setOverdueCreditModule((OverdueCreditModule) module);
    }
}
