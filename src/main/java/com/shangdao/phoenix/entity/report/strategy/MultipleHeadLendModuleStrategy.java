...
import com.shangdao.phoenix.entity.report.module.MultipleHeadLendModule;
 ...
 * @date 2018/04/03
 */
public class MultipleHeadLendModuleStrategy extends SingleAPIModuleStrategy {
    private static final Logger logger = LoggerFactory.getLogger(MultipleHeadLendModuleStrategy.class);

    private final static long API_ID = 17L;

    public MultipleHeadLendModuleStrategy(Report report, SupplyAPIRepository supplyAPIRepository, APISearchRepository apiSearchRepository) {
    ...
    public BaseModule analyseData() {
        MultipleHeadLendModule multipleHeadLendModule = new MultipleHeadLendModule(Color.SUCCESS);
        if (apiResponse != null) {
            if ("0000".equals(apiResponse.get("rc").toString())) {
                ...
                for (Object o : list) {
                    ...
                    if ("E".equals(blackRiskItem.get("blackRiskType"))) {
                ...
                if (blackRisks.size() > 0) {
                    multipleHeadLendModule.setColor(Color.DANGER);
                }
                multipleHeadLendModule.setBlackRisks(blackRisks);
            } else {
                multipleHeadLendModule.setColor(Color.ERROR);
            }
        } else {
            multipleHeadLendModule.setColor(Color.TIMEOUT);
        }
    ...
    public void setModuleIntoReport(BaseModule module) {
        report.setMultipleHeadLendModule((MultipleHeadLendModule) module);
    }
}
